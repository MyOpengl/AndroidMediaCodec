...
public class CircularEncoderBuffer {
    ...
    public CircularEncoderBuffer(int bitRate, int frameRate, int desiredSpanSec) {
        ...
        if (VERBOSE) {
            Log.d(TAG, "CBE: bitRate=" + bitRate + " frameRate=" + frameRate + " desiredSpan=" + desiredSpanSec + ": dataBufferSize=" + dataBufferSize + " metaBufferCount=" + metaBufferCount);
        }
    }
    ...
    private boolean canAdd(int size) {
        ...
        if (size > freeSpace) {
            if (VERBOSE) {
                Log.v(TAG, "ran out of data (tailStart=" + tailStart + " headStart=" + headStart + " req=" + size + " free=" + freeSpace + ")");
            }
            return false;
        }

        if (VERBOSE) {
            Log.v(TAG, "OK: size=" + size + " free=" + freeSpace + " metaFree=" + ((mMetaTail + metaLen - mMetaHead) % metaLen - 1));
        }
